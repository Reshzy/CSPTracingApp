...
- App commit/build and renderer path: HEAD `c976eac` (`feature/init`; Prompt 26/26b files uncommitted). Prompt 26 adds `RecoverGpuResources` / Simulate GPU loss (no TDR) and `deviceLoss=` status distinct from `GetDeviceRemovedReason`; capture Stop on DXGI device-lost copy; overlay HWND recreate reapplies WDA_EXCLUDEFROMCAPTURE before show. Prompt 26b: `TrackingSessionPolicy::MarkUnavailable` calls `DropKeyframe()`. Debug `out/build/windows-debug/Debug/TracingApp.exe` (3025408 bytes, 2026-09-26 13:31:00), `TracingApp.TrackingSessionTests.exe` (830464 bytes, 2026-09-26 13:31:00). Previous: Win32 control window plus WS_EX_LAYERED overlay HWND presented with `CreateSwapChainForHwnd` (`presentPath=dxgi-hwnd`, default WDA_EXCLUDEFROMCAPTURE; temporary WDA_NONE positive-control latch; `SetLayeredWindowAttributes(LWA_ALPHA)`; no `WS_EX_NOREDIRECTIONBITMAP`; no ULW; no DirectComposition) and a separate ordinary WS_OVERLAPPEDWINDOW ReferenceWindow (WDA_NONE, DXGI swap chain); skip-image-present diagnostic keeps the magenta/yellow test marker on the overlay swap-chain RTV; ImageRenderer draws into that same RTV using overlay-local 2x3 affine constants plus RS scissor; after each owned WGC copy, canvas `RoiReadback` packs BGRA CPU rows (not zero-copy) with `roiSrc=applied-mapped` or full-content fallback; a second Navigator-kind `RoiReadback` copies only an applied-mapped Navigator ROI (`navRoiSrc=none` until Apply Nav ROI maps through measured capture-to-client offset; no full-content fallback); `TrackingSession` owns a worker and publishes immutable `TransformSnapshot`s with `parity`/`driftRms`/`reacquire`/`secondary` plus fusion mode/reject/src/weight fields; origin keyframe is kept for the session and an optional second validated anchor is used only to relocalize when origin residual is worse; `VisualTracker` evaluates None/FlipX/FlipY remapped hypotheses then an orientation-preserving fit; after `ApplyObservation` accepts, a persistent `TransformFusion` fuses overlay-local visual `LastRelative` (VisualOnly default) or hybrid Navigator/Input/UIA samples; displayed `M_DS` uses policy `LastRelative` unless a hybrid observer actually contributed; `CaptureSession::LastRoiBuffer` still feeds canvas tracking; `LastNavigatorRoiBuffer` packed BGRA is bound as `NavigatorPixelFormat::Bgra32` into `NavigatorObserver::Observe` after PumpHandoff without Detach. `NavigatorObserver` has no OCR; Last() is copied into fusion when Hybrid is on. `InputObserver` mouse Raw Input is registered after `CreateWindowExW` with `RIDEV_INPUTSINK` and no `RIDEV_NOLEGACY` / no digitizer; prediction affects overlay only when Hybrid fuse is on. `AccessibilityObserver` remains a read-only UIA capability probe (no Invoke/SetValue); Last() is copied as optional fusion dropout. Control window Hybrid fuse checkbox defaults unchecked. Prompt 24 `MetricsCollector` samples existing FormatReport/Snapshot/Info/Affinity fields into a 256-sample ring with p50/p95/max; OBS verify combo defaults NOT RUN and is never auto-set from affinity; Image replay checkbox defaults off (local `out/replay`, 32 MiB). shaders compiled with discovered `fxc.exe` `10.0.26100.8249` at `C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\fxc.exe`; capture preview stayed off; Release not rebuilt this step
...
- CSP version/locale/theme/workspace and drawing device: CLIPStudioPaint.exe 5.0.0 (`FileVersion` 5.0.0.0) at `C:\Program Files\CELSYS\CLIP STUDIO 1.5\CLIP STUDIO PAINT\CLIPStudioPaint.exe` pid 11744 during Prompt 22 (CLIPStudio.exe launcher pid 47232). Probe locale from `GetUserDefaultLocaleName`=`en-US`; Windows UI culture `en-GB`. Theme/workspace/pen NOT RECORDED.
...
- M6 visual/parity tracking and confidence safety: PARTIAL (Prompt 16a/16b ROI readback PASS; Prompt 17 synthetic ORB+RANSAC estimator PASS with measured textured errors below 0.2 px / 0.05 deg on 320x240 fixtures; Prompt 18a TrackingSession GoogleTests PASS; Prompt 18b worker-path textured translation / blank GoogleTests PASS and live titled-CSP ROI feed + hide-on-Lost PASS; Prompt 19a synthetic FlipX/FlipY / combined / two-flip≡180 / symmetric-ambiguous / prolonged-outlier / return-to-keyframe GoogleTests PASS; Prompt 19b session hysteresis / Pause-on-ParityAmbiguous / return-to-keyframe worker / secondary-anchor relocalization / separate drift vs reacquire GoogleTests PASS; Prompt 25 synthetic replay 8 GoogleTests PASS with independent landmarks: pan median=0.214 p95=0.326, rotate median=0.207 p95=0.423, combined median=0.191 p95=0.283, stroke median=0.079 p95=0.157, FlipX median=0.027 p95=0.044 (ROI px); isolated 1.12 scale on seed 0xC0FF is `parity-ambiguous`; reacquire 2653 ms vs 1 s NOT MET. Settled live Tracking on default 80,80 640x480 ROI did not hold in 18b; live CSP flips NOT RUN.)
- M7 hybrid observers/fusion: PARTIAL (Prompt 20a synthetic NavigatorObserver GoogleTests PASS; Prompt 20b CaptureSession Navigator ROI feed + packed-BGRA Observe GoogleTests PASS; Prompt 21 InputObserver 10 GoogleTests PASS and control smoke rawInput=ok predEnabled=no until Apply mapping, Enable+wheel with no session is reject=no-session not Predicted; Prompt 22 AccessibilityObserver 11 GoogleTests PASS; live titled CLIP STUDIO PAINT 5.0.0.0 UIA probe is `unsupported` with no numeric zoom/rotation/Navigator values in a 400 ms / 137-node prefix, adapter disabled; Prompt 23a TransformFusion 11 GoogleTests PASS including visual-only vs hybrid synthetic replay (hybrid accepted at t=0 from input prediction; visual-only waited until t=80; contradicting visual at t=80 published tx=0 not the prediction; confident-wrong count did not increase); Prompt 23b TrackingSession live-observer wiring PASS (VisualOnly ignores predicted input; hybrid predicted pan +10 px without bumping sequence; visual tx=0 corrects prediction; Navigator Missing / unsupported UIA dropout keeps visual pose; wrong-generation observer does not move snapshot; Hybrid fuse checkbox defaults off); live CSP Navigator appearance/pan/zoom/hidden/themes NOT RUN; live CSP input-prediction vs pan/zoom NOT RUN; live same-replay visual-only vs hybrid overlay on CSP NOT RUN)
- M8 integration/performance/final recording regression: PARTIAL (Prompt 24 bounded metrics/event/replay GoogleTests PASS; Prompt 25 synthetic estimator/fusion replay PASS; Prompt 26 Debug configure/build/test PASS plus 3 new TrackingSession fault GoogleTests and simulated GPU-loss control smoke `deviceLoss=simulated removed=0x00000000` overlay hidden affinity readback 0x11; Prompt 26b `MarkUnavailable` DropKeyframe GoogleTest PASS; live titled CSP pid 11744 resize-while-capturing PASS (`client-size-changed`, overlay hidden, WGC lastSeq 30 recreates=5); shutdown-while-capturing leftover=False; real driver reset NOT RUN; titled CSP WM_CLOSE/reopen NOT RUN; 30-minute CSP run, memory trend, and final OBS Display Capture regression remain NOT RUN)
...
- [ ] Visual tracking tested with textured drawing and new strokes. (17: synthetic textured pan/zoom/rotate and stroke-outlier fixtures PASS; 18b worker GoogleTest translation ~8,5 px PASS. 25: synthetic replay pan/rotate/combined/stroke landmark median/p95 under 2/5 ROI px; isolated 1.12 scale on seed 0xC0FF `parity-ambiguous`; stroke median=0.079 p95=0.157. Live titled CLIP STUDIO PAINT pid 36888: ROI feed enqueue PASS; one run keyframe inliers=683 conf=0.333 then Lost hide; another keyframe then poor-coverage Lost. Overlay did not stay locked through pan/zoom. MASTER 2 px not claimed on CSP.)
...
- [ ] Blank/symmetric scenes and ambiguous flips degrade/hide safely. (17: synthetic blank/gradient reject `blank-or-low-texture`. 19a: left-right symmetric identity is `parity-ambiguous` conf=0. 19b: `SubmitEstimateForTest` ParityAmbiguous -> Paused, M_DS frozen until Resume/Resync; one FlipX rejected then second accepted; worker return-to-keyframe restores identity within 2.5 px; blank does not promote secondary. 25: blank lastReject=`blank-or-low-texture`, sequence frozen, origin held, last accepted confidence may remain; FlipX first hop does not publish parity; later hops accept FlipX (median 0.027 px). Live CSP blank canvas / CSP flips NOT RUN.)
- [ ] Navigator disappearance and optional observer disablement handled. (21: Enable pred with no session then mouse_event wheel -> `reject=no-session` pending=no assumedCspApplied=no; Disable pred path keeps mapping. 22: Probe UIA with no session stays disabled; titled CSP 5.0.0.0 probe `uia=unsupported` foundZoom/Rot/Nav=no observed=none adapter disabled. 23a: Missing Navigator + Unsupported UIA dropout keeps visual pose; 23b: HybridNavigatorAndUiaDropoutKeepsVisualPose GoogleTest PASS; live CSP Navigator hide / input-pred-on-CSP NOT RUN.)
- [ ] Stale/wrong-generation observations cannot move overlay. (18a GoogleTest: seq 4 after 5 ignored; targetGeneration mismatch ignored; snapshot M_DS unchanged. 18b live: Lost hide=yes with overlay visibility=Hide reason=target-unusable after stale age; WGC continued. 23a: wrong-generation and stale visual/input FusionTests reject; delayed visual t=10 after accepted t=80 does not overwrite last pose. 23b: HybridWrongGenerationObserverDoesNotMoveSnapshot GoogleTest PASS.)
- [x] Document/layout changes invalidate calibration or require explicit stop/resync. (15a/16b plus 26b live titled CLIP STUDIO PAINT pid 11744 hwnd `0x50DC4`: SW_RESTORE then shrink 2376x1290 / grow 2496x1370 / SW_MAXIMIZE while WGC Running; `invalidation=client-size-changed`, tracking Detach/Unattached, `overlayHiddenOnCaptureLoss=yes`, overlay visible=no until re-Apply ROI. Mixed-DPI still absent.)
- [ ] Target restart, device-recovery path and shutdown leave no orphan overlay. (26: Simulate GPU loss rebuilt overlay HWND `0x310FCC` visible=no affinity matchExclude=yes. 26b: CloseMainWindow while capturing titled CSP ExitCode=0 leftover=False overlayAlive=False. Real GetDeviceRemovedReason TDR NOT RUN. Titled CLIP STUDIO PAINT WM_CLOSE/reopen NOT RUN — document left open.)
- [ ] Debug previews disabled during recording; pixel recording opt-in only. (24: control smoke CapturePreview HWND visible=False, status `preview=off replay=off`; Image replay checkbox BST_UNCHECKED. Live recording session with metrics NOT RUN.)
...
```text
Step / milestone: 21 / M7 Optional input prediction
Date / commit: 2026-09-26 / working tree on feature/init HEAD bfa17ae (uncommitted Prompt 21 files)
Status: PASS for Debug configure/build/test, 10 InputPrediction GoogleTests, and control-window smoke (rawInput=ok after CreateWindow; predEnabled=no until Apply mapping; Enable+wheel with no session is reject=no-session, not Predicted). Live CSP foreground prediction vs pan/zoom/pen NOT RUN (no CLIPStudioPaint.exe at the later smoke; fusion not connected).
Changed files:
  src/platform/InputObserver.h
...
Build command + exit code:
  ...
  exit 0
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 2655232 bytes (2026-09-26 10:53:40)
  TracingApp.InputPredictionTests.exe 956416 bytes (2026-09-26 10:51:05)
  TracingApp.BootstrapTests.exe 112640 bytes (2026-09-26 10:54:20)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 14, passed 14, failed 0
  TracingApp.InputPredictionTests --gtest_list_tests: 10 cases (DisabledByDefaultIgnoresWheelSample, MappingNotAppliedKeepsDisabledAfterEnable, EnabledVersusDisabledSameWheelSample, UnknownGestureAndNonMouseProduceNoPrediction, WrongForegroundIgnoresAndDoesNotAssumeCspApplied, GenerationChangeDropsPending, PredictionExpiresWithin100msWithoutVisual, ContradictoryVisualDiscardsPending, AgreeingVisualConfirmsAndClears, RejectsNonFiniteMapping)
  TracingApp.InputPredictionTests 0.01s in final ctest
  TracingApp.VisualTrackerTests 6.46s; TracingApp.TrackingSessionTests 1.79s (unchanged this step)
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe class TracingAppControlWindow. After CreateWindowExW, RegisterRawInputDevices mouse usage 0x01/0x02 with RIDEV_INPUTSINK succeeded (status rawInput=ok err=0). Launch: predEnabled=no mapping=no session=no pred src=disabled. BN_CLICKED Apply mapping (id 1061): mapping=yes predEnabled=no. BM_SETCHECK Enable pred (id 1062) then BN_CLICKED: predEnabled=yes mapping=yes session=no. SetForegroundWindow control + mouse_event MOUSEEVENTF_WHEEL +120: pred src=disabled reject=no-session pending=no assumedCspApplied=no (did not invent a Predicted transform). CloseMainWindow ExitCode=0 leftover=false. Status saved to out/manual/prompt21-control-smoke.txt.
  Get-CimInstance Win32_Process CLIPStudioPaint.exe / CLIPStudio.exe during the later smoke: none. An earlier process query in this step saw CLIPStudio.exe pid 47232 and CLIPStudioPaint.exe pid 11744, but that instance was not selected and no wheel was sent while it was foreground. Did not cover InputProbe, did not register pen HID, did not apply prediction to overlay.
  Exact steps if a titled CLIP STUDIO PAINT document is available: select the PAINT row; Apply mapping (defaults z=1.1 p=1.0 rdeg=15); Enable pred; confirm TracingApp status predEnabled=yes session=yes; focus CSP and wheel/middle-drag — status may show src=predicted then expire within 100 ms or confirm/contradict from a new tracking snapshot; overlay must not jump from input this step; Disable pred and repeat — no pending prediction. Confirm mouse/wheel still reach CSP in tracing mode. Pen/pressure remains a later hardware check. Close TracingApp with CloseMainWindow.
Expected / actual:
  expected: disabled by default; mapping required before Enable; Raw Input observe-only (no NOLEGACY); wrong foreground / no session / unknown gesture / generation change / 100 ms expiry / contradictory visual covered by GoogleTest; live enabled vs disabled on CSP if hardware allows; overlay unfused
  actual: tests PASS as listed. Control smoke matches rawInput=ok, mapping gate, no-session wheel not Predicted. Live CSP prediction comparison NOT RUN. TrackingSession.cpp was not modified. Fusion remains Prompt 23.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.InputPredictionTests.exe
  out/manual/prompt21-control-smoke.txt
Measured samples / p50 / p95 / max where applicable:
  GoogleTest expiry: Tick at 99 ms still Predicted pending; Tick at 100 ms Expired
  live CSP observation-to-present age: NOT RUN
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  RegisterRawInputDevices during WM_CREATE is too early for RIDEV_INPUTSINK; registration runs after CreateWindowExW
  no CLIPStudioPaint process at the recorded smoke; live CSP pan/zoom/pen comparison NOT RUN
  input prediction is not fused into TrackingSession or overlay placement (Prompt 23)
  RIDEV_INPUTSINK delivers WM_INPUT while unfocused; samples are dropped unless enabled and are not stored as a HID log
  MASTER 2 px / 0.5 deg / 0.5% targets are not claimed
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 22 — Optional accessibility capability probe.
  2. Optional later: live CSP input-prediction vs pan/zoom with a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball.
Blocker or next prompt: 22 — Optional accessibility capability probe (do not auto-run)
```

```text
Step / milestone: 22 / M7 Optional accessibility capability probe
Date / commit: 2026-09-26 / working tree on feature/init HEAD 5af472d (uncommitted Prompt 22 files)
Status: PASS for Debug configure/build/test, 11 ObserverCapability GoogleTests, control smoke (Probe UIA with no session stays disabled), and live titled CLIP STUDIO PAINT UIA inspection. Observed outcome is Unsupported (valid): no numeric zoom/rotation/Navigator values in a bounded walk; adapter left disabled; not fused into tracking.
Changed files:
  src/platform/AccessibilityObserver.h
...
Build command + exit code:
  cmake --build --preset windows-debug
  exit 0 after fixing undeclared UIA_E_ELEMENTNOTAVAILABLE (used documented HRESULT 0x80040201)
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 2750464 bytes (2026-09-26 11:04:43)
  TracingApp.ObserverCapabilityTests.exe 1042944 bytes (2026-09-26 11:04:46)
  TracingApp.BootstrapTests.exe 112640 bytes (2026-09-26 11:04:24)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 15, passed 15, failed 0
  TracingApp.ObserverCapabilityTests --gtest_list_tests: 11 cases (UnprobedDefaultIsDisabled, EmptyTreeIsUnsupportedAndStaysDisabled, ProviderOpenFailureIsErrorAndStaysDisabled, ZoomRangeValueIsSupportedOnlyForMatchingGeneration, GenerationChangeDropsCacheAndDisables, TargetClosedDropsCacheAndDisables, UnavailableNodeMidWalkIsStaleNotFabricatedValue, KeywordNameWithoutValueRemainsUnsupported, MaxNodesClassifiesBoundedPrefixWithoutHanging, NoSessionInspectKeepsDisabled, MarkTargetClosedClearsEnabledAdapter)
  TracingApp.ObserverCapabilityTests 0.03s in final ctest
  TracingApp.VisualTrackerTests 6.43s; TracingApp.TrackingSessionTests 1.74s (unchanged this step)
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe class TracingAppControlWindow. Launch: uia=unprobed enabled=no uiaSession=no. BN_CLICKED Probe UIA (id 1063) with no selection: status "Probe UIA: no session... Adapter stays disabled" and uiaSession=no uiaEnabled=no. Refresh listed three [PAINT] PID 11744 rows; selected titled "CLIP STUDIO PAINT" (client physical origin=(1920,0) size=2560x1439 dpi=96); uiaSession=yes still unprobed. BN_CLICKED Probe UIA: worker finished in ~421 ms with "UIA probe finished. Observed properties only; no CSP-support claim." Status: uia=unsupported reject=unsupported enabled=no foundZoom=no foundRot=no foundNav=no nodes=137 truncated=yes locale=en-US observed=none uiaEnabled=no uiaSession=yes uiaProbing=no. CloseMainWindow ExitCode=0 leftover=false. Saved to out/manual/prompt22-control-smoke.txt.
  CLIPStudioPaint.exe FileVersion 5.0.0.0 pid 11744; CLIPStudio.exe pid 47232. GetUserDefaultLocaleName=en-US; Windows UI culture en-GB. An untitled [PAINT] palette HWND (231x263) was also unsupported (134 nodes, truncated) in an earlier pass; the titled document HWND is the recorded observation. Did not Invoke/SetValue; TrackingSession was not fed.
  Exact steps to repeat: launch TracingApp; Probe UIA before Select and confirm disabled/no-session; Select the titled CLIP STUDIO PAINT row (not an owned Color Wheel); Probe UIA; record uia=/found*/observed=/locale= and CSP FileVersion. Unsupported with enabled=no is a pass for this optional adapter.
Expected / actual:
  expected: disabled/unprobed by default; no-session probe keeps disabled; fake-tree GoogleTests cover unsupported/provider-error/stale/generation/keyword-without-value/bounded walk; live CSP documents only observed properties; adapter not used for tracking
  actual: tests PASS as listed. Live titled CSP walk found no numeric zoom/rotation/Navigator RangeValue/Value matches in 137 raw-view nodes before the 400 ms bound (truncated=yes). That is unsupported, not a claim that later nodes cannot contain such values. Overlay/tracking unchanged.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.ObserverCapabilityTests.exe
  out/manual/prompt22-control-smoke.txt
Measured samples / p50 / p95 / max where applicable:
  titled CSP probe wait ~421 ms; nodesVisited=137 truncated=yes (limit 256 nodes / 400 ms)
  GoogleTest maxNodes=2 visits 2 then one extra Next to set truncated
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  400 ms bound truncates a large CSP tree; this step does not prove absence of properties beyond the visited prefix
  no Invoke/SetValue; no OCR; observed=none so no numeric zoom percent or rotation degrees from UIA
  AccessibilityObserver is not fused into TrackingSession or overlay placement (Prompt 23)
  MASTER 2 px / 0.5 deg / 0.5% targets are not claimed
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 23 — Timestamped hybrid fusion.
  2. Optional later: live CSP input-prediction vs pan/zoom with a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; deeper UIA walk if a later step raises the bound.
Blocker or next prompt: 23 — Timestamped hybrid fusion (do not auto-run)
```

```text
Step / milestone: 23a / M7 Timestamped hybrid fusion (engine + visual-only session hook)
Date / commit: 2026-09-26 / working tree on feature/init HEAD 1af1154 (uncommitted Prompt 23a files)
Status: PASS for Debug configure/build/test, 11 Fusion GoogleTests, TrackingSessionTests still PASS after visual-only identity fuse, and control-window launch/close. Live same-replay visual-only vs hybrid on CSP NOT RUN (no CLIPStudioPaint.exe). Hybrid accepted on synthetic replay only; not claimed for live CSP. 23b remaining.
Changed files:
  src/tracking/TransformFusion.h
...
Build command + exit code:
  cmake --build --preset windows-debug
  exit 0 after replacing VisualParityName in FormatFusionResult with a local parity label so FusionTests does not link OpenCV
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 2782208 bytes (2026-09-26 11:19:04)
  TracingApp.FusionTests.exe 984576 bytes (2026-09-26 11:19:06)
  TracingApp.BootstrapTests.exe 112640 bytes (2026-09-26 11:04:24)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 16, passed 16, failed 0
  TracingApp.FusionTests --gtest_list_tests: 11 cases (VisualOnlyIdentityPass, ObserverDropoutKeepsVisual, DelayedVisualDoesNotOverwriteNewer, AngleWraparoundIsSmallStep, ContradictoryParityRejectsWithoutAveraging, InputCorrectionPrefersVisualAnchor, RelocalizationReplacesWrongPrediction, SameReplayHybridImprovesLatencyWithoutConfidentWrong, WrongGenerationAndStaleAreRejected, NavigatorUvMapsWithoutInventingZoomPercent, StaleInputDoesNotContribute)
  TracingApp.FusionTests 0.01s in final ctest
  TracingApp.VisualTrackerTests 7.23s; TracingApp.TrackingSessionTests 2.15s (PASS after identity fuse hook)
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe. After 4s EnumWindows for pid 44864: TracingAppControlWindow hwnd 0x1A0F7E, Overlay 0xF0FA2, Reference 0x170F90, CapturePreview 0x150F84. CloseMainWindow=True ExitCode=0 leftover=false. Saved to out/manual/prompt23a-control-smoke.txt.
  Get-CimInstance Win32_Process CLIPStudioPaint.exe / CLIPStudio.exe: none. Did not compare live visual-only vs hybrid on a CSP document. Did not change control-window status fields (no 23b source-contribution UI).
  Exact steps if a titled CLIP STUDIO PAINT document is available later (23b): select PAINT; Start Capture; Start tracking; Apply Nav ROI and Enable pred; pan/zoom; compare overlay with pred disabled vs enabled on the same motion; fusion must not raise confident-wrong alignment. This substep cannot perform that comparison because live observers are not fed.
Expected / actual:
  expected: TransformFusion mixes translation/log-scale/unwrapped angle within one parity; rejects stale/wrong-generation/delayed/parity-conflict; input yields to visual/Navigator anchors; visual-only identity fuse does not change TrackingSessionTests; synthetic hybrid replay improves early recovery without increasing confident-wrong; live CSP comparison if hardware allows
  actual: tests PASS as listed. Control smoke launched and closed. Live CSP hybrid comparison NOT RUN. TrackingSession.cpp calls Fuse with visual only (empty observers). Hybrid is accepted for the synthetic replay only.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.FusionTests.exe
  out/manual/prompt23a-control-smoke.txt
Measured samples / p50 / p95 / max where applicable:
  wraparound GoogleTest: Wrap((-179 deg) - (179 deg)) = 2 deg
  synthetic replay: visual-only acceptedEarly=0; hybrid acceptedEarly=1 at t=0; both t=80 tx=10 when visual agrees; both t=80 tx=0 when visual contradicts prediction; hybrid confident-wrong=0
  live CSP observation-to-present age: NOT RUN
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  no CLIPStudioPaint process this step; live visual-only vs hybrid CSP replay NOT RUN
  TrackingSession.h and main.cpp unchanged; live Navigator/Input/UIA samples are not fused (23b)
  UIA remains unsupported on CSP 5.0.0.0 from Prompt 22; fusion treats that as dropout
  no Kalman filter; gated weighted mix only
  MASTER 2 px / 0.5 deg / 0.5% targets are not claimed
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 23b — feed live Navigator/Input into TransformFusion; expose source contributions on snapshot/control status; optional hybrid toggle defaulting to visual fallback.
  2. Optional later: live CSP input-prediction vs pan/zoom with a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; deeper UIA walk if a later step raises the bound.
Blocker or next prompt: 23b — live observer fusion wiring (do not auto-run)
```

```text
Step / milestone: 23b / M7 Timestamped hybrid fusion (live observer wiring)
Date / commit: 2026-09-26 / working tree on feature/init HEAD 2eb8a8f (uncommitted Prompt 23a/23b files)
Status: PASS for Debug configure/build/test, 11 Fusion GoogleTests unchanged, 24 TrackingSession GoogleTests (5 new hybrid-wiring cases) PASS, and control-window launch/close with Hybrid fuse default unchecked. Live same-replay visual-only vs hybrid overlay on a titled CSP document NOT RUN (CLIPStudioPaint.exe pid 11744 was present but this substep did not Start Capture / Start tracking / Apply Nav ROI / Enable pred / Hybrid on). Hybrid accepted on session GoogleTests and 23a synthetic replay only; not claimed for live CSP.
Changed files:
  src/tracking/TrackingSession.h
  src/tracking/TrackingSession.cpp
  src/app/main.cpp
...
Configure command + exit code:
  ...
  reused opencv4[calib3d,core,intrinsics,thread]:x64-windows@4.12.0#7 and gtest:x64-windows@1.17.0#3
  Found OpenCV 4.12.0; TracingApp fxc: C:/Program Files (x86)/Windows Kits/10/bin/10.0.26100.0/x64/fxc.exe
  SDK 10.0.26100.0; binaryDir out/build/windows-debug
Build command + exit code:
  cmake --build --preset windows-debug
  exit 0
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 2799104 bytes (2026-09-26 11:50:55)
  TracingApp.TrackingSessionTests.exe 793600 bytes (2026-09-26 11:50:53)
  TracingApp.FusionTests.exe 984576 bytes (2026-09-26 11:19:06)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 16, passed 16, failed 0
  TracingApp.FusionTests --gtest_list_tests: 11 cases (unchanged from 23a)
  TracingApp.TrackingSessionTests --gtest_list_tests: 24 cases including VisualOnlyIgnoresPredictedInput, HybridPredictedPanAcceptsEarly, HybridVisualCorrectsInputPrediction, HybridNavigatorAndUiaDropoutKeepsVisualPose, HybridWrongGenerationObserverDoesNotMoveSnapshot
  TracingApp.TrackingSessionTests 1.74s PASS; TracingApp.FusionTests 0.01s PASS; TracingApp.VisualTrackerTests 6.35s
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe. After 4s EnumWindows for pid 42832: TracingAppControlWindow hwnd 0x250FD2 visible, Overlay 0x4C0EE6, Reference 0x130F4A, CapturePreview 0x121032. CloseMainWindow=True ExitCode=0 leftover=False. Hybrid fuse is BS_AUTOCHECKBOX created BST_UNCHECKED (visual-only default). Saved to out/manual/prompt23b-control-smoke.txt.
  Get-CimInstance Win32_Process: CLIPStudioPaint.exe pid 11744 and CLIPStudio.exe pid 47232 were running. Did not select a PAINT HWND, Start Capture, Start tracking, Apply Nav ROI, Enable pred, or toggle Hybrid fuse on a live document. Did not compare overlay with pred/hybrid disabled vs enabled on the same CSP motion.
  Exact steps if repeating the live comparison: select titled CLIP STUDIO PAINT; Start Capture; apply canvas ROI; Start tracking; Apply Nav ROI; Apply mapping; Enable pred; pan/zoom the same motion with Hybrid fuse unchecked then checked; fusion must not raise confident-wrong alignment; missing Navigator / Disable Nav / pred off must keep visual/manual fallback.
Expected / actual:
  expected: live Navigator/Input copied into TransformFusion; VisualOnly default ignores observers; Hybrid predicted pan can move overlay early without bumping visual sequence; visual anchor corrects prediction; Navigator Missing and unsupported UIA dropout keep visual pose; wrong-generation observers do not move snapshot; source contributions on snapshot/status; Hybrid fuse checkbox default off; live CSP comparison if hardware allows
  actual: tests PASS as listed. Control smoke launched and closed. Live CSP hybrid overlay comparison NOT RUN. Displayed M_DS stays policy LastRelative unless a hybrid observer actually contributed.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.TrackingSessionTests.exe
  out/build/windows-debug/Debug/TracingApp.FusionTests.exe
  out/manual/prompt23b-control-smoke.txt
Measured samples / p50 / p95 / max where applicable:
  HybridPredictedPanAcceptsEarly: overlay origin +10.0 px at sequence 1; usedInput=true usedVisual=false
  HybridVisualCorrectsInputPrediction: after predicted +10, OkEstimate(0,0) restored keyframe origin; usedVisual=true usedInput=false sequence=2
  live CSP observation-to-present age: NOT RUN
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  CLIPStudioPaint.exe pid 11744 was running; live visual-only vs hybrid CSP overlay comparison NOT RUN
  UIA remains unsupported on CSP 5.0.0.0 from Prompt 22; fusion treats that as dropout
  no Kalman filter; gated weighted mix only
  MASTER 2 px / 0.5 deg / 0.5% targets are not claimed
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 24 — Diagnostics and privacy-aware evidence.
  2. Optional later: live CSP input-prediction vs pan/zoom with Hybrid fuse on a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; deeper UIA walk if a later step raises the bound.
Blocker or next prompt: 24 — Diagnostics and privacy-aware evidence (do not auto-run)
```

```text
Step / milestone: 24 / M8 Diagnostics and privacy-aware evidence
Date / commit: 2026-09-26 / working tree on feature/init HEAD a223f5d (uncommitted Prompt 24 files)
Status: PASS for Debug configure/build/test, 11 Metrics GoogleTests, and control-window launch/close with obsVerify=NOT RUN, preview=off, replay=off while affinityOk=yes. Live long-run / CSP capture-with-metrics / 30-minute memory trend NOT RUN (CLIPStudioPaint.exe pid 11744 was present; this step did not Start Capture or Start tracking). CV/fusion stage microseconds are not exported by TrackingSession; trackingLagMs is UI-observed sequence lag only.
Changed files:
  src/diagnostics/Metrics.h
...
Build command + exit code:
  cmake --build --preset windows-debug
  exit 0 after MetricsTests size-cap fixture used non-hex 'z' so SanitizeLogText would not collapse the payload
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 3008512 bytes (2026-09-26 12:12:19)
  TracingApp.MetricsTests.exe 1145856 bytes (2026-09-26 12:12:49)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 17, passed 17, failed 0
  TracingApp.MetricsTests --gtest_list_tests: 11 cases (EmptyReturnsZero, ZeroToNinetyNineNearestRank, SampleWindowDoesNotGrowPast256, EventLogDropsOldestAt128, AffinityOkDoesNotCertifyObs, StripsFullPrivatePathToLeaf, OmitsPixelDumpAndRawInputPayload, DefaultDisabledWritesZeroBytes, OptInStopsAtInjectedSizeLimit, ReadsDroppedBoundAndFirstMapWait, NumericOnlyNoPaths)
  TracingApp.MetricsTests 0.01s PASS; TracingApp.VisualTrackerTests 6.38s; TracingApp.TrackingSessionTests 1.72s
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe. After 4s EnumWindows for pid 13696: TracingAppControlWindow hwnd 0x2C0F9E visible; Overlay 0x140F4A hidden; Reference 0x6109C hidden; CapturePreview 0xD0F96 visible=False. Status snippet: metrics samples=2 ... affinityOk=yes obsVerify=NOT RUN (user-recorded; affinity API is not certification) preview=off replay=off. CloseMainWindow=True ExitCode=0 leftover=False. Saved to out/manual/prompt24-control-smoke.txt.
  Get-CimInstance Win32_Process: CLIPStudioPaint.exe pid 11744 and CLIPStudio.exe pid 47232 were running. Did not select a PAINT HWND, Start Capture, Start tracking, or enable Image replay. Did not change OBS verify away from NOT RUN.
  Exact steps if repeating a live long-run: select titled CLIP STUDIO PAINT; Start Capture; Apply ROI; Start tracking; leave Enable capture preview unchecked; leave Image replay unchecked unless writing local out/replay; leave OBS verify at NOT RUN unless a saved Display Capture was inspected; confirm metrics samples stay at most 256 and the private overlay is not shown in Capture Preview.
Expected / actual:
  expected: bounded metrics with p50/p95/max; OBS verification user-recorded and not auto-certified from affinity; default logs omit pixels/full paths/raw input; image replay opt-in with size cap; capture preview off by default; live long-run if hardware allows
  actual: tests PASS as listed. Control smoke launched and closed with obsVerify=NOT RUN while affinityOk=yes, preview=off, replay=off, CapturePreview HWND not visible. Live CSP metrics/long-run NOT RUN.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.MetricsTests.exe
  out/manual/prompt24-control-smoke.txt
Measured samples / p50 / p95 / max where applicable:
  Percentile GoogleTest 0..99: p50=49 p95=94 max=99 (nearest-rank ceil)
  300 injected samples collapse to 256; 200 events retain 128 (oldest event-72)
  ReplayRecorder 100-byte cap: first 61-byte numeric line ok; second rejected SizeLimit
  control smoke: metrics samples=2 captureAgeMs/staging/trackingLag all p50=0 (no capture)
  live CSP captureAge/stagingWait/trackingLag: NOT RUN
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  CLIPStudioPaint.exe pid 11744 was running; live capture/tracking metrics and 30-minute trend NOT RUN
  TrackingSession does not export CV vs fusion stage timers; trackingLagMs is UI-thread sequence lag
  MASTER 2 px / 0.5 deg / 0.5% / 100 ms observation-to-present targets are not claimed
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 25 — Deterministic replay regression suite.
  2. Optional later: live CSP input-prediction vs pan/zoom with Hybrid fuse on a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; live metrics long-run; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; deeper UIA walk if a later step raises the bound.
Blocker or next prompt: 25 — Deterministic replay regression suite (do not auto-run)
```

```text
Step / milestone: 25 / M6 Deterministic replay regression suite
Date / commit: 2026-09-26 / working tree on feature/init HEAD c7a13f4 (uncommitted Prompt 25 files)
Status: PASS for Debug configure/build/test and 8 synthetic TrackingReplay GoogleTests. Isolated pan/rotate/combined/stroke landmark median/p95 met MASTER 2 px / 5 px and 0.5° / 0.5% relative scale on 320x240 ROI pixels. Isolated 1.12 uniform scale on seed 0xC0FF is a recorded failing scenario (`parity-ambiguous`, MASTER not asserted). Worker reacquire 2653 ms vs MASTER 1 s NOT MET. Observation-to-present age N/A. Real CSP acceptance NOT RUN (no CLIPStudioPaint process).
Changed files:
  tests/replay/SequenceGenerator.h
  tests/replay/SequenceGenerator.cpp
  tests/replay/TrackingReplayTests.cpp
...
Build command + exit code:
  ...
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 3008512 bytes (2026-09-26 12:12:19)
  TracingApp.TrackingReplayTests.exe 735744 bytes (2026-09-26 12:42:08)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 18, passed 18, failed 0
  TracingApp.TrackingReplayTests --gtest_list_tests: 8 cases (SequenceMetadataIsSeededAndIndependentOfEstimator, SettledTexturedFramesMeetMasterPixelTargets, CombinedFromKeyframeIsRecordedSeparately, MissingFrameGapDoesNotJumpThenBlankLosesConfidence, LostThenReacquireWithinOneSecond, FlipXHysteresisThenNoConfidentWrongParity, VisualOnlyIgnoresObserversOnGeneratedPan, HybridContradictionDoesNotIncreaseConfidentWrong)
  TracingApp.TrackingReplayTests 18.43s PASS; TracingApp.VisualTrackerTests 6.37s; TracingApp.TrackingSessionTests 1.75s
Manual setup and exact actions:
  Synthetic only: ran TracingApp.TrackingReplayTests.exe and full ctest. Did not launch TracingApp.exe against CSP. Get-CimInstance Win32_Process found no CLIPStudioPaint.exe / CLIPStudio.exe this step. No OBS recording. Exact CSP steps remain those in Prompt 18b/24 if repeating live tracking.
Expected / actual:
  expected: seeded sequences with pan/zoom/rotation/reflection, brush strokes, missing frames, blank, reacquisition feeding the real estimator/fusion pipeline; landmark GT independent of the estimator; MASTER bounds where applicable; synthetic marked separately from CSP
  actual: 8 GoogleTests PASS. Landmark n=25. Isolated scale on seed 0xC0FF paused as parity-ambiguous (seq stayed 1). Blank rejected blank-or-low-texture with frozen origin; last accepted confidence may remain. FlipX first hop held parity=none; later hops published flipX. Hybrid contradicting Navigator did not increase confident-wrong vs visual-only (tx stayed ~7.84). MASTER recovery 1 s NOT MET (2653 ms). Age p95 N/A.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.TrackingReplayTests.exe
  tests/replay/README.md
Measured samples / p50 / p95 / max where applicable (synthetic 320x240 ROI px, seed 0xC0FF, 25 landmarks, nearest-rank ceil):
  pan: median=0.214 p95=0.326 max=0.370 scaleRel=0.00097 degErr=0.071
  scale: FAILING SCENARIO parity-ambiguous; median=12 p95=17.3 (MASTER not asserted)
  rotate: median=0.207 p95=0.423 max=0.436 scaleRel=0.00202 degErr=0.035
  combined: median=0.191 p95=0.283 max=0.311 scaleRel=0.00042 degErr=0.045
  stroke: median=0.079 p95=0.157 max=0.167 scaleRel=2.7e-5 degErr=0.038
  gap-pan: median=0.214 p95=0.326 max=0.370
  reacquire: median=0.214 p95=0.326 elapsedMs=2653 (MASTER <=1s NOT MET)
  flip-x: median=0.027 p95=0.044 max=0.050
  fusion: visualAgree tx=7.841 hybridAgree tx=7.938 visualContra tx=7.841 hybridContra tx=7.841 wrong=0
  observation-to-present age p95: N/A (no present path)
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  no CLIPStudioPaint process; live CSP replay/acceptance NOT RUN
  isolated 1.12 scale on seed 0xC0FF is parity-ambiguous (not tolerance-tuned)
  worker reacquire runs multiple ORB fits per frame; 2653 ms vs 1 s
  MASTER 2 px / 5 px figures here are 320x240 ROI pixels, not 1080p/1440p CSP
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 26 — Integration fault handling.
  2. Optional later: live CSP input-prediction vs pan/zoom with Hybrid fuse on a selected PAINT HWND; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; live metrics long-run; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; deeper UIA walk if a later step raises the bound.
Blocker or next prompt: 26 — Integration fault handling (do not auto-run)
```

```text
Step / milestone: 26 / M8 Integration fault handling
Date / commit: 2026-09-26 / working tree on feature/init HEAD c976eac (uncommitted Prompt 26 files)
Status: PASS for Debug configure/build/test and simulated GPU-loss recovery. Real GetDeviceRemovedReason driver reset NOT RUN (no TDR invoked). Live CSP close/reopen/resize-while-capturing NOT RUN (no CLIPStudioPaint process).
Changed files:
  src/app/main.cpp
  src/capture/CaptureSession.cpp
...
Build command + exit code:
  ...
  exit 0
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 3008512 bytes (2026-09-26 13:14:58)
  TracingApp.TrackingSessionTests.exe 830464 bytes (2026-09-26 13:15:05)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  exit 0
  discovered 18, passed 18, failed 0
  TracingApp.TrackingSessionTests --gtest_list_tests: 25 cases including new MarkUnavailableHidesAndRejectsFrames, BeginCalibratedAfterUnavailableRequiresFreshKeyframe, PendingThenUnavailableThenStopDoesNotDeadlock
  TracingApp.TrackingSessionTests 1.80s PASS; TracingApp.TrackingReplayTests 18.47s; TracingApp.VisualTrackerTests 6.40s
Manual setup and exact actions:
  Control smoke: Start-Process out/build/windows-debug/Debug/TracingApp.exe pid 3084176-era hwnd then 3084176/6950630. Posted WM_COMMAND id=1067 (Simulate GPU loss). Status after rebuild saved to out/manual/prompt26-simulate-gpu-loss.txt. CloseMainWindow=True ExitCode=0 leftover=False.
  Get-CimInstance Win32_Process found no CLIPStudioPaint.exe / CLIPStudio.exe. Did not Start Capture against CSP, close/reopen CSP, or resize while capturing. Did not invoke a system GPU reset / TDR.
  Exact CSP steps if repeating: select titled CLIP STUDIO PAINT; Start Capture; Apply ROI; Start tracking; close CSP (expect hide + generation invalid); reopen and reselect; resize repeatedly; exit TracingApp while capturing. Click Simulate GPU loss once and confirm deviceLoss=simulated overlay hidden until Apply ROI.
Expected / actual:
  expected: target-close / layout invalidation / capture-loss / device-removal / shutdown invalidate generations before teardown, rebuild GPU dependents in order, reapply affinity, hide until fresh calibration; inject lifecycle failures; distinguish simulated vs real driver reset; no destructive GPU reset
  actual: tests PASS as listed. Simulated recovery PASS: deviceLoss=simulated (not a driver reset) removed=0x00000000 overlayHiddenOnCaptureLoss=yes overlay visible=no hwnd=0x0000000000310FCC affinity set=ok readback=0x00000011 matchExclude=yes retryCount=0 state=Ready. Real GetDeviceRemovedReason NOT RUN. Live CSP lifecycle NOT RUN.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.TrackingSessionTests.exe
  out/manual/prompt26-simulate-gpu-loss.txt
Measured samples / p50 / p95 / max where applicable:
  control smoke after simulate: metrics samples=3 captureAge/staging/trackingLag p50=0 (no capture)
  live CSP capture-loss / resize / close: NOT RUN
  real device-removed HRESULT: NOT RUN
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  no CLIPStudioPaint process; live close/reopen/resize-while-capturing NOT RUN
  real driver reset / TDR NOT RUN; simulated path is not GetDeviceRemovedReason evidence
  TrackingSessionPolicy::MarkUnavailable (header-inline) does not DropKeyframe; owned buffers are cleared in TrackingSession.cpp; snapshot hasKeyframe may remain until BeginCalibrated
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 27 — Known-transform Windows fixture.
  2. Optional later: live CSP close/reopen/resize while capturing; real GPU TDR (do not force); live CSP input-prediction vs pan/zoom with Hybrid fuse; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; live metrics long-run; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball; header DropKeyframe on MarkUnavailable (26b).
Blocker or next prompt: 27 — Known-transform Windows fixture (do not auto-run)
```

```text
Step / milestone: 26b / M8 MarkUnavailable DropKeyframe + live CSP resize-while-capturing
Date / commit: 2026-09-26 / working tree on feature/init HEAD c976eac (uncommitted Prompt 26+26b files)
Status: PASS for DropKeyframe-on-Unavailable GoogleTest, live titled CLIP STUDIO PAINT capture, restore/shrink/grow/re-maximize while capturing, and TracingApp exit-while-capturing. Real GetDeviceRemovedReason / TDR NOT RUN (not invoked). Titled-frame CSP WM_CLOSE/reopen NOT RUN (document left open).
Changed files:
  src/tracking/TrackingSession.h
  tests/unit/TrackingSessionTests.cpp
  project-docs/VALIDATION.md
Configure command + exit code:
  ...
  SDK 10.0.26100.0; binaryDir out/build/windows-debug
Build command + exit code:
  cmake --build --preset windows-debug --target TracingApp TracingApp.TrackingSessionTests
  exit 0
  MSBuild 18.10.1-1.26427.6+3cd27c13e
  TracingApp.exe 3025408 bytes (2026-09-26 13:31)
  TracingApp.TrackingSessionTests.exe 830464 bytes (2026-09-26 13:31)
Test command + discovered/passed/failed counts:
  ctest --preset windows-debug --output-on-failure
  first pass: discovered 18, passed 17, failed 1 (TrackingReplay.LostThenReacquireWithinOneSecond Degraded vs Lost after Tick; independent of MarkUnavailable)
  rerun TracingApp.TrackingReplayTests --gtest_filter=TrackingReplay.LostThenReacquireWithinOneSecond: PASS 3086 ms reacquireMs=2695
  TracingApp.TrackingSessionTests 1.94s PASS including EXPECT_FALSE(hasKeyframe) after MarkUnavailable
Manual setup and exact actions:
  Get-Process CLIPStudioPaint pid 11744 MainWindowTitle empty MainWindowHandle 69244 (SysShadow 0x10E7C). EnumWindows titled HWND 0x50DC4 title='CLIP STUDIO PAINT' ownerless maximized 2576x1456 at (1912,-8) on DISPLAY1/R27qe. CLIPStudio.exe launcher pid 47232 titled CLIP STUDIO.
  Start-Process TracingApp.exe pid 41264 class TracingAppControlWindow hwnd 0x3E107A. Refresh listed row[13]=[PAINT] PID 11744 CLIPStudioPaint.exe | CLIP STUDIO PAINT. Select, Start Capture, Apply ROI 80,80 640x480, Start tracking.
  First SetWindowPos on the maximized frame did not change size. SW_RESTORE then shrink/grow/SW_MAXIMIZE while WGC Running. CloseMainWindow while capturing (CSP left open). Did not invoke a GPU TDR.
Expected / actual:
  expected: live CSP process is the titled painting HWND, not Get-Process.MainWindowTitle; resize-while-capturing invalidates calibration and hides overlay; MarkUnavailable drops the keyframe; shutdown leaves no orphan overlay; TDR not forced; titled WM_CLOSE optional if a document is open
  actual: after Select+Start Capture WGC Running lastSeq=1 contentSize=2576x1456. After ROI+track: Calibrating overlay placement (2000,80) 640x480. SW_RESTORE: maximized=False rect=2576x1440 geomGen=2 lastSeq=11 overlay visible=no overlayHiddenOnCaptureLoss=yes invalidation=client-size-changed tracking Unattached. Shrink 2376x1290 geomGen=3 lastSeq=17 recreates=3 pending=2. Grow 2496x1370 geomGen=4 lastSeq=23 recreates=4. Re-maximize 2576x1456 geomGen=5 lastSeq=30 recreates=5 overlay still hidden. CloseMainWindow=True ExitCode=0 leftover=False overlayAlive=False. CSP hwnd 0x50DC4 still alive maximized. MarkUnavailable snapshot hasKeyframe=no.
Evidence paths (local, no private artwork committed; out/ is gitignored):
  out/build/windows-debug/Debug/TracingApp.exe
  out/build/windows-debug/Debug/TracingApp.TrackingSessionTests.exe
  out/manual/prompt26b-live-csp.txt
  out/manual/prompt26b-live-csp-resize.txt
Measured samples / p50 / p95 / max where applicable:
  WGC lastSeq 1 (idle maximized) then 11/17/23/30 across restore/shrink/grow/re-maximize
  ROI ring pending<=2 droppedBound=0 recreates=5 copyUs 2–4 us mapWaitUs 1–3 us (full-content-fallback after invalidation)
  Close TracingApp leftover=False in ~66 ms after CloseMainWindow=True
Known limitations / unavailable hardware:
  cmake/ctest not on PATH; invoked via VS 2026 bundled binaries
  Release configure/build/test NOT RUN this step
  Get-Process.MainWindowTitle is not a valid CSP-alive check; use EnumWindows / TracingApp [PAINT] rows
  titled CLIP STUDIO PAINT WM_CLOSE/reopen NOT RUN (document and Color Wheel/Sub View left open)
  real driver reset / TDR NOT RUN; simulated path is not GetDeviceRemovedReason evidence
  idle WGC may stay at lastSeq=1 until geometry change; applied-mapped ROI CPU hasCpu was no until restore generated frames
  TrackingReplay.LostThenReacquireWithinOneSecond can flake Degraded vs Lost on a single Tick
  mixed-DPI and negative-origin monitors still absent
Manual checklist remaining:
  1. Prompt 27 — Known-transform Windows fixture.
  2. Optional later: titled CSP WM_CLOSE/reopen while capturing (only with a disposable document); real GPU TDR (do not force); live CSP input-prediction vs pan/zoom with Hybrid fuse; live CSP Navigator pan/zoom/rotation/themes/hidden; live CSP flip; live metrics long-run; DXGI DDA and Windows 10 (1903+) OBS methods; per-pixel overlay transparency; CSP landmark eyeball.
Blocker or next prompt: 27 — Known-transform Windows fixture (do not auto-run)
```
