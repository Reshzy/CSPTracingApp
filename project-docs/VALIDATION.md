...
- [ ] Navigator disappearance and optional observer disablement handled. (21: Enable pred with no session then mouse_event wheel -> `reject=no-session` pending=no assumedCspApplied=no; Disable pred path keeps mapping. Live CSP Navigator hide / input-pred-on-CSP NOT RUN.)
