...
- App commit/build and renderer path: HEAD `2da36f1` (`feature/init`; Prompt 16a RoiReadback files uncommitted); Win32 control window plus WS_EX_LAYERED overlay HWND presented with `CreateSwapChainForHwnd` (`presentPath=dxgi-hwnd`, default WDA_EXCLUDEFROMCAPTURE; temporary WDA_NONE positive-control latch; `SetLayeredWindowAttributes(LWA_ALPHA)`; no `WS_EX_NOREDIRECTIONBITMAP`; no ULW; no DirectComposition) and a separate ordinary WS_OVERLAPPEDWINDOW ReferenceWindow (WDA_NONE, DXGI swap chain); skip-image-present diagnostic keeps the magenta/yellow test marker on the overlay swap-chain RTV; ImageRenderer draws into that same RTV using overlay-local 2x3 affine constants plus RS scissor; after each owned WGC copy, `RoiReadback` copies a bounded canvas region to a two-slot staging ring and packs BGRA CPU rows (not zero-copy); shaders compiled with discovered `fxc.exe` `10.0.26100.8249` at `C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\fxc.exe`; capture preview stayed off; Debug `out/build/windows-debug/Debug/TracingApp.exe` (2364416 bytes, 2026-09-25 23:14:12), `TracingApp.RoiTests.exe` (975872 bytes, 2026-09-25 23:14:16), `TracingApp.CalibrationTests.exe` and `TracingApp.InputProbe.exe`; Release not rebuilt this step
...
- M6 visual/parity tracking and confidence safety: NOT RUN (Prompt 16a ROI clip/stride/pending tests PASS; live CSP copyUs/mapWaitUs/pending/resize NOT RUN; applied canvas ROI wiring is 16b; OpenCV estimator is prompt 17)
...
- [ ] Visual tracking tested with textured drawing and new strokes. (16a: synthetic ROI clip/pack/pending PASS; live CSP ROI readback timings NOT RUN)
- [ ] Bounded ROI readback copy/map wait and pending<=2 on CSP resize.
