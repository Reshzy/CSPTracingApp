...
- M8 integration/performance/final recording regression: PARTIAL (Prompt 24 bounded metrics/event/replay GoogleTests PASS and control smoke `preview=off` `replay=off` `obsVerify=NOT RUN` with `affinityOk=yes`; Prompt 25 synthetic estimator/fusion replay PASS and is not a present-path latency measurement; 30-minute CSP run, memory trend, and final OBS Display Capture regression remain NOT RUN)
