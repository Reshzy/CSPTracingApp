...
- [ ] Blank/symmetric scenes and ambiguous flips degrade/hide safely. (17: synthetic blank/gradient reject `blank-or-low-texture`. 19a: left-right symmetric identity is `parity-ambiguous` conf=0; TrackingSession Pauses on that reject and does not publish a new M_DS. 18b worker blank after keyframe -> Degraded, M_DS unchanged. Live CSP blank canvas / CSP flips NOT RUN.)
